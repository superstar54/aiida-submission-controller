...
prepare_input1 = wg.add_task(prepare_input, name="prepare_input",
                             ...
                             submitted_group_label="submitted_structures1",
...
find_next1.set_context({"should_run": "should_run", "structure": "structure"})
while1 = wg.add_task("While", name="While", conditions=["should_run"])
submit_job1 = wg.add_task(submit_job, name="submit_job", code=pw_code,
                        structure="{{structure}}",
                        protocol="fast",
                        submitted_group=prepare_input1.outputs["submitted_group"])
find_next2 = wg.add_task(find_next, name="find_next2", to_submit_group=prepare_input1.outputs["to_submit_group"],
                        submitted_group=prepare_input1.outputs["submitted_group"])
find_next2.waiting_on.add(submit_job1)
find_next2.set_context({"should_run": "should_run", "structure": "structure"})
while1.children.add([submit_job1, find_next2])
while1.waiting_on.add(find_next1)
